...
function Header() {
  ...
  const { linkedin, github, email } = socialMediaUrl;
  const toggleClass = () => {
    setIsOpen(!isOpen);
  };
    ...
    <header className="container mx-auto md:flex justify-between py-2 max-width">
      ...
      <nav className={`${!isOpen ? "hidden" : ""} text-center md:flex justify-between`}>
        <ul className="dark:text-light-content font-medium md:flex items-center md:space-x-5 md:mr-10">
          ...
          <li>
            <a href="https://drive.google.com/file/d/1yh9eVZh34BcRqwsF4t48Hya4txGzKXA_/view?usp=sharing" target="_blank" rel="noopener noreferrer" onClick={toggleClass}>
              Resume
            </a>
          </li>
        </ul>
        <ul className="flex justify-evenly items-center my-5 md:my-0 md:space-x-5 md:mr-5">
          <li>
            <a href={`mailto:${email}`} target="_blank" rel="noreferrer noopener">
              <svg
                className="w-8 h-8 dark:fill-light-heading fill-dark-heading"
                width="32"
                height="32"
                viewBox="0 0 177 177"
                xmlns="http://www.w3.org/2000/svg"
              >
                ...
                <path
                  ...
                  className="fill-white dark:fill-dark-heading"
                />
              </svg>
            </a>
          </li>
